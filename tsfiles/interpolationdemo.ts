...
var widget: InterpolationWidget;

class InterpolationWidget extends EasyResizeWidget {
	constructor(obj) {
		super(obj);
...
function getZs(sel): number[] {
	return $(sel).val().split("\n").filter(z => z.length > 0).map(parseFloat);
}

function drawTicksCanvas(alphasbetas: number[], bpf: BPF, N: number, widgetCanvas: HTMLCanvasElement) {
	var tickscanvas = <HTMLCanvasElement>$("#tickscanvas")[0];
	...
	ticksctx.beginPath(); ticksctx.moveTo(0, tN / 2); ticksctx.moveTo(tN, tN / 2); ticksctx.stroke();
	ticksctx.beginPath(); ticksctx.moveTo(tN / 2, 0); ticksctx.moveTo(tN / 2, tN); ticksctx.stroke();
	ticksctx.save();
...
function checkAlphasBetas(alphas: number[], betas: number[], alphasbetas: number[]): boolean {
...
$(function () {
	...
	$("#interpolate").click(function () {
		...
		var betas: number[] = getZs("#betas");
		var alphasbetas = alphas.concat(betas).sort(o);

		if (!checkAlphasBetas(alphas, betas, alphasbetas)) { return; }
...
function fillTable(alphasbetas: number[], bpf: BPF) {
